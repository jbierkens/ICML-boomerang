...
using Plots
using CSV
function addEntry!(df, sampler, dim, ess, ess_log_density, runtime, time_horizon, refresh_rate, σ_squared)
    push!(
        df,
        Dict(
            :sampler => sampler,
...
function runall(numb_of_exp = 20, over_dimension = [1,10,100], over_sigma_squared = 10.0 .^ (-3.0:0.5:3.0),
    ...
    for dim in over_dimension
        ...
        Q = LinearAlgebra.symmetric(Matrix{Float64}(I, dim, dim),:U)
            for _ in 1:numb_of_exp
                for σ_squared in over_sigma_squared
                ...
                addEntry!(df, "Boomerang", dim, ess, ess_log_density, runtime, time_horizon, refresh_rate, σ_squared)
            ...
            addEntry!(df, "BPS", dim, ess, ess_log_density, runtime, time_horizon, refresh_rate, NaN)
        end
    end
    df
end
...
over_sigma_squared = 2.0 .^ (-1.0:0.2:1.0)
time_horizon = 10000.0
df = runall(numb_of_exp, over_dimension, over_sigma_squared, time_horizon)
CSV.write("data-by-sigma.csv", df)
